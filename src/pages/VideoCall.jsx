...
// Add this to your server-side socket event handlers

// Handle reconnection requests
socket.on('request-reconnection', ({ with: targetId, roomId }) => {
  console.log(`Reconnection requested from ${socket.id} to ${targetId} in room ${roomId}`);
  ...
  if (!user) {
    ...
    socket.emit('error-message', { message: 'User not found' });
    return;
  }
  ...
  if (!targetSocket) {
    console.error(`Target socket ${targetId} not found`);
    socket.emit('error-message', { message: 'Target user disconnected' });
    return;
  }
  
  // Send reconnection request to the target
  targetSocket.emit('reconnect-request', {
    from: socket.id,
    name: user.userName
  });
  
  console.log(`Reconnection request sent from ${user.userName} (${socket.id}) to ${targetId}`);
});

// Also add this to your room-users event to ensure it includes all necessary data
socket.on('request-room-users', ({ roomId }) => {
  const roomUsers = users.filter(u => u.roomId === roomId && u.socketId !== socket.id);
  console.log(`Room users requested for ${roomId}:`, roomUsers);
